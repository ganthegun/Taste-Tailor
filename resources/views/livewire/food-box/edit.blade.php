<div>
    ...
    <div class="grid md:grid-cols-2 lg:grid-cols-2 gap-6">
        ...
        <div>
            ...
            <div class="grid md:grid-cols-3 lg:grid-cols-3 gap-6 p-6">
                @foreach ($menus as $menu)
                    <div
                        @if (collect($selected_menus)->contains('id', $menu['id']))
                    ...
                    >
                        ...
                        <div class="p-4">
                            ...
                            <div class="text-sm text-gray-800 dark:text-gray-300 mt-2">
                                {{ __('Price: RM') . number_format($menu['price'], 2) }}
                            </div>
                            <div class="text-sm text-gray-800 dark:text-gray-300 mt-2">
                                {{ __('Nutrient:') }} {{ $menu['nutrient'] }}
                            </div>
                        </div>
                    </div>
                @endforeach
        ...
        <form wire:submit="update({{ $id }})" class="space-y-6">
            ...
            <flux:input :label="__('Selected Menu')" type="text" value="{{ implode(', ', array_column($selected_menus, 'name')) ?? '' }}" required readonly/>
            ...
            <x-action-message class="me-3 flex justify-end" on="food-box-updated">
                {{ __('Updated.') }}
            </x-action-message>
        </form>
    </div>
